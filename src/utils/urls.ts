export const STRAPI_API_TOKEN = process.env.NEXT_PUBLIC_STRAPI_API_TOKEN;
export const STRIPE_API_TOKEN = process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY;

export const API_URL = process.env.NEXT_PUBLIC_STRAPI_API_URL || "http://127.0.0.1:1337";

export const CLIENT_URL = process.env.NEXT_PUBLIC_CLIENT_URL || 'https://consorcio-ac-electrica-sac.vercel.app';

export const OPENIA_URL = process.env.NEXT_PUBLIC_OPENIA_URL;

export const MERCADO_ACCESS_TOKEN = process.env.NEXT_ACCESS_TOKEN_MERCADO;